    ...
    public class NotesController : ControllerBase
    {
        ...
        public ActionResult<List<Note>> FilterNotes(string text, int priority)
        {
            try
            {
                ...
                {
                    return BadRequest("Filter parameters are requried and must be valid");
                }
                if (priority > 3)
                {
                    return BadRequest("Priority must be between 1 and 3");
                }
                var notes = StaticDb.Notes.Where(x => x.Text.ToLower().Contains(text) && (int)x.Priority == priority).ToList();
                if(notes.Count > 0)
                {
                    return Ok(notes);
                }
